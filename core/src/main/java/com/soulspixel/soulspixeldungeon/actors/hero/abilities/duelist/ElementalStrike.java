...
package com.soulspixel.soulspixeldungeon.actors.hero.abilities.duelist;
...
import com.soulspixel.soulspixeldungeon.actors.buffs.CounterBuff;
...
import com.soulspixel.soulspixeldungeon.items.weapon.curses.Annoying;
import com.soulspixel.soulspixeldungeon.items.weapon.curses.Dazzling;
import com.soulspixel.soulspixeldungeon.items.weapon.curses.Displacing;
import com.soulspixel.soulspixeldungeon.items.weapon.curses.Explosive;
import com.soulspixel.soulspixeldungeon.items.weapon.curses.Friendly;
import com.soulspixel.soulspixeldungeon.items.weapon.curses.Polarized;
import com.soulspixel.soulspixeldungeon.items.weapon.curses.Sacrificial;
import com.soulspixel.soulspixeldungeon.items.weapon.curses.Wayward;
	...
	private void perCharEffect(ConeAOE cone, Hero hero, Char primaryTarget, Weapon.Enchantment ench) {
		...
		if (ench == null) {
			for (Char ch : affected){
				ch.damage(Math.round(powerMulti* Char.combatRoll(6, 12)), ElementalStrike.this, null);
			}
		...
		//*** Kinetic ***
			...
			if (storedKineticDamage > 0) {
				for (Char ch : affected) {
					if (ch != primaryTarget) {
						ch.damage(Math.round(storedKineticDamage * 0.4f * powerMulti), ench, null);
					}
				}
		...
		//*** Projecting ***
			...
			for (Char ch : affected){
				if (ch != primaryTarget) {
					ch.damage(Math.round(hero.damageRoll() * 0.3f * powerMulti), ench, null);
				}
			}
		...
		//*** Grim ***
			...
			for (Char ch : affected){
				if (ch != primaryTarget) {
					...
					if (Random.Float() < chance*powerMulti){
						ch.damage( ch.HP, Grim.class, null);
		...
		//*** Polarized ***
			...
			for (Char ch : affected){
				if (Random.Float() < 0.5f*powerMulti){
					ch.damage(Char.combatRoll(24, 36), ElementalStrike.this, null);
				}
			}
