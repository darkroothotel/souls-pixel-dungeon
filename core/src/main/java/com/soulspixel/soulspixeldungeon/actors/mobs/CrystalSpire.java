...
import com.soulspixel.soulspixeldungeon.sprites.CrystalSpireSprite;
		...
		if (!targetedCells.isEmpty()){
			...
			for (int i : cellsToAttack){
				...
				if (ch != null && !(ch instanceof CrystalWisp || ch instanceof CrystalSpire)){
					...
					ch.damage(dmg, new SpireSpike(), DamageType.MAGIC);
					...
					if (isAlive()) {
						...
						for (Char ch : Actor.chars()){
							if (fieldOfView[ch.pos]) {
								if (ch instanceof CrystalGuardian) {
									ch.damage(ch.HT, new SpireSpike(), DamageType.MAGIC);
								}
