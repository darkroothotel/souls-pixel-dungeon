...
import com.soulspixel.soulspixeldungeon.actors.buffs.Foresight;
import com.soulspixel.soulspixeldungeon.actors.buffs.HoldFast;
...
import com.soulspixel.soulspixeldungeon.actors.buffs.Levitation;
...
import com.soulspixel.soulspixeldungeon.items.artifacts.CapeOfThorns;
...
import com.soulspixel.soulspixeldungeon.items.bags.MagicalHolster;
import com.soulspixel.soulspixeldungeon.items.journal.Guidebook;
import com.soulspixel.soulspixeldungeon.items.keys.CrystalKey;
...
public class Hero extends Char {
	...
	public void setBonfireDepth(int depth){
		lastbonfiredepth = depth;
	}

	public void setBonfirePos(int pos){
		lastbonfirepos = pos;
	}

	public int getBonfireDepth(){
		return lastbonfiredepth;
	}

	public int getBonfirePos(){
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		lastbonfiredepth = bundle.getInt(LB_DEPTH);
		lastbonfirepos = bundle.getInt(LB_POS);
	...
	private boolean actAlchemy( HeroAction.Alchemy action ) {
		...
		if (Dungeon.level.distance(dst, pos) <= 1) {
			...
			SoulsPixelDungeon.switchScene(AlchemyScene.class);
			return false;
	...
	public boolean handle( int cell ) {
		...
		//TODO perhaps only trigger this if hero is already adjacent? reducing mistaps
			...
			curAction = new HeroAction.Unlock( cell );
				...
				&& (Dungeon.depth < 76 || Dungeon.level.getTransition(cell).type == LevelTransition.Type.REGULAR_ENTRANCE) ) {
