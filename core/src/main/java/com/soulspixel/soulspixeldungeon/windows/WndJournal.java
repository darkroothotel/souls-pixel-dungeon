		...
		private void updateList(){
			...
			if (!keys.isEmpty()){
				...
				for(Notes.Record rec : keys){
					...
					ScrollingListPane.ListItem item = new ScrollingListPane.ListItem( icon, Integer.toString(rec.depth()), Messages.titleCase(rec.desc()));
			...
			if (!landmarks.isEmpty()){
				...
				for (Notes.Record rec : landmarks) {
					Image icon = Icons.get(Icons.STAIRS);
					if(rec instanceof Notes.LandmarkRecord){
					...
					ScrollingListPane.ListItem item = new ScrollingListPane.ListItem( icon,
		...
		private void updateList(){
			...
			for (Document doc : Document.values()){
				...
				ScrollingListPane.ListItem item = new ScrollingListPane.ListItem(
					...
					public boolean onClick(float x, float y) {
						if (inside( x, y ) && found) {
							SoulsPixelDungeon.scene().addToFront( new WndDocument( doc ));
							return true;
						} else {
							return false;
						}
