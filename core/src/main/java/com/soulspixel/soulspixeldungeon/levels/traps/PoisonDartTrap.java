...
import com.soulspixel.soulspixeldungeon.items.weapon.missiles.darts.PoisonDart;
			...
			protected boolean act() {
				...
				if (target != null) {
					...
					if (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[target.pos]) {
						((MissileSprite) SoulsPixelDungeon.scene().recycle(MissileSprite.class)).
								reset(pos, finalTarget.sprite, new PoisonDart(), new Callback() {
									@Override
									public void call() {
										int dmg = Char.combatRoll(4, 8) - finalTarget.drRoll();
										finalTarget.damage(dmg, PoisonDartTrap.this, Char.DamageType.PIERCE);
						...
						finalTarget.damage(Char.combatRoll(4, 8) - finalTarget.drRoll(), PoisonDartTrap.this, Char.DamageType.PIERCE);
						Buff.affect( finalTarget, Poison.class ).set( poisonAmount() );
						return true;
					}
				} else {
