...
import com.soulspixel.soulspixeldungeon.windows.WndChallenges;
import com.soulspixel.soulspixeldungeon.windows.WndHeroInfo;
...
import com.soulspixel.soulspixeldungeon.windows.WndTextInput;
...
public class HeroSelectScene extends PixelScene {
		...
		infoButton = new IconButton(Icons.get(Icons.INFO)){
			...
			protected void onClick() {
				...
				if (cls != null) {
					...
					SoulsPixelDungeon.scene().addToFront(w);
				}
			}
	...
	protected void onBackPressed() {
		if (btnExit.active){
			SoulsPixelDungeon.switchScene(TitleScene.class);
		} else {
			super.onBackPressed();
		}
	}
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				SoulsPixelDungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
			...
			if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
				StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
					...
					protected void onClick() {
						...
						SoulsPixelDungeon.scene().addToFront( new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
								...
								if (positive && seed != -1){
									...
									for (GamesInProgress.Info info : GamesInProgress.checkAll()){
										if (info.customSeed.isEmpty() && info.seed == seed){
											SPDSettings.customSeed("");
											icon.resetColor();
											SoulsPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
											return;
										}
									}
				...
				StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
					...
					protected void onClick() {
						...
						if (diff > 24*HOUR){
							SoulsPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
							return;
						}

						for (GamesInProgress.Info game : GamesInProgress.checkAll()){
							if (game.daily){
								SoulsPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
								return;
							}
						}
						...
						SoulsPixelDungeon.scene().addToFront(new WndOptions(
								icon,
				...
				StyledButton challengeButton = new StyledButton(Chrome.Type.BLANK, Messages.get(WndChallenges.class, "title"), 6){
					@Override
					protected void onClick() {
						SoulsPixelDungeon.scene().addToFront(new WndChallenges(SPDSettings.challenges(), true) {
