...
import com.soulspixel.soulspixeldungeon.windows.WndSettings;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		StyledButton btnPlay = new StyledButton(GREY_TR, Messages.get(this, "enter")){
			...
			protected void onClick() {
				if (GamesInProgress.checkAll().size() == 0){
					...
					SoulsPixelDungeon.switchNoFade( StartScene.class );
				}
			}
		...
		StyledButton btnRankings = new StyledButton(GREY_TR,Messages.get(this, "rankings")){
			@Override
			protected void onClick() {
				SoulsPixelDungeon.switchNoFade( RankingsScene.class );
			}
		};
		...
		StyledButton btnBadges = new StyledButton(GREY_TR, Messages.get(this, "badges")){
			@Override
			protected void onClick() {
				SoulsPixelDungeon.switchNoFade( BadgesScene.class );
			}
		};
		...
		if (landscape()) {
			...
			align(btnPlay);
			btnRankings.setRect(btnPlay.left(), btnPlay.bottom()+ GAP, (btnPlay.width()*.33f)-.5f, BTN_HEIGHT);
		...
		BitmapText version = new BitmapText( "v" + SoulsPixelDungeon.instance.getCorrectedGameVersion(), pixelFont);
		version.measure();
	...
	private static class NewsButton extends StyledButton {
		...
		public void update() {
			...
			if (unreadCount > 0){
				textColor(ColorMath.interpolate( 0xFFFFFF, Window.SPX_COLOR, 0.5f + (float)Math.sin(Game.timeTotal*5)/2f));
			}
		}
		...
		protected void onClick() {
			super.onClick();
			SoulsPixelDungeon.switchNoFade( NewsScene.class );
		}
	}
				...
				SoulsPixelDungeon.scene().addToFront( new WndOptions(
					...
					protected void onSelect(int index) {
						if (index == 0) {
							...
							SoulsPixelDungeon.switchNoFade( ChangesScene.class );
						}
					}
				});
	...
	private static class SettingsButton extends StyledButton {
		...
		protected void onClick() {
			...
			SoulsPixelDungeon.scene().add(new WndSettings());
		}
	}
	...
	private static class SupportButton extends StyledButton{
		...
		@Override
		protected void onClick() {
			SoulsPixelDungeon.switchNoFade(SupporterScene.class);
		}
	}
}
