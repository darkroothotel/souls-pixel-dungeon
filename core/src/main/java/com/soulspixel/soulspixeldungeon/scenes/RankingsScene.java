...
import com.soulspixel.soulspixeldungeon.windows.WndDailies;
import com.soulspixel.soulspixeldungeon.windows.WndRanking;
	...
	public void create() {
		...
		if (Rankings.INSTANCE.records.size() > 0) {
			...
			if (Rankings.INSTANCE.totalNumber >= Rankings.TABLE_SIZE) {
				...
				label.setHightlighting(true, Window.SPX_COLOR);
		...
		if (Rankings.INSTANCE.latestDaily != null) {
			IconButton btnDailies = new IconButton(Icons.CALENDAR.get()) {
				...
				protected void onClick() {
					SoulsPixelDungeon.scene().addToFront(new WndDailies());
				}
