...
import com.soulspixel.soulspixeldungeon.windows.WndHardNotification;
...
public class WelcomeScene extends PixelScene {
	...
	private static final int LATEST_UPDATE = SoulsPixelDungeon.v2_4_0;
	...
	public void create() {
		...
		if (!triedCleaningTemp && FileUtils.cleanTempFiles()){
			add(new WndHardNotification(Icons.get(Icons.WARNING),
				...
				public void hide() {
					...
					SoulsPixelDungeon.resetScene();
				}
			});
			return;
		}

		if (SoulsPixelDungeon.versionCode == previousVersion && !SPDSettings.intro()) {
		...
		StyledButton okay = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				if (previousVersion == 0 || SPDSettings.intro()){
					...
					SPDSettings.version(SoulsPixelDungeon.versionCode);
		...
		if (previousVersion != 0 && !SPDSettings.intro()){
			StyledButton changes = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(TitleScene.class, "changes")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					SoulsPixelDungeon.switchScene(ChangesScene.class);
				}
			};
		...
		} else if (previousVersion <= SoulsPixelDungeon.versionCode) {
	...
	private void updateVersion(int previousVersion){
		...
		if (previousVersion < LATEST_UPDATE){
			...
			if (previousVersion <= SoulsPixelDungeon.v2_0_2){
			...
			if (previousVersion <= SoulsPixelDungeon.v2_3_2){
