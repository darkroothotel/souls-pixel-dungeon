...
import com.soulspixel.soulspixeldungeon.effects.Effects;
...
import com.soulspixel.soulspixeldungeon.levels.traps.TenguDartTrap;
	...
	public void onZap(Ballistica bolt) {
		...
		for (int i  : PathFinder.NEIGHBOURS8){
			...
			if (ch != null){
				...
				if (ch.alignment != Char.Alignment.ALLY) ch.damage(damageRoll(), this, Char.DamageType.STRIKE);
		...
		if (ch != null){
			wandProc(ch, chargesPerCast());
			ch.damage(damageRoll(), this, Char.DamageType.STRIKE);
		...
		Actor.add(new Pushing(ch, ch.pos, newPos, new Callback() {
			public void call() {
				...
				if (finalCollided && ch.isActive()) {
					ch.damage(Char.combatRoll(finalDist, 2*finalDist), new Knockback(), Char.DamageType.STRIKE);
					if (ch.isActive()) {
