...
import com.soulspixel.soulspixeldungeon.windows.WndSadGhost;
...
public class TrinketCatalyst extends Item {
	...
	public static class Recipe extends com.soulspixel.soulspixeldungeon.items.Recipe {
		...
		public Item brew(ArrayList<Item> ingredients) {
			...
			SoulsPixelDungeon.scene().addToFront(new WndTrinket(newCata));
			return null;
		}
			...
			for (int i = 0; i < NUM_TRINKETS; i++){
				ItemButton btnReward = new ItemButton(){
					...
					protected void onClick() {
						SoulsPixelDungeon.scene().addToFront(new RewardWindow(item()));
					}
				};
				...
				RedButton btnConfirm = new RedButton(Messages.get(WndSadGhost.class, "confirm")){
					...
					protected void onClick() {
						...
						if (cata != null) {
							...
							((AlchemyScene)SoulsPixelDungeon.scene()).craftItem(null, item);
						}
					}
				};
